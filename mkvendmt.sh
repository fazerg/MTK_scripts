...
UNPACKBOOTIMG=~/bin/unpack-MT65xx.pl
...
then
    ...
    unpack-MT65xx.pl $BOOTIMAGEFILE > /dev/null
    ...
    printf "%x" `cat $TMPDIR/$BOOTIMAGEFILE-base` > $TMPDIR/$BOOTIMAGEFILE-base
    ...
    cp $TMPDIR/$BOOTIMAGEFILE-kernel.img $DEVICE_DIR/kernel
    popd > /dev/null
else
    mkdir -p $DEVICE_DIR
